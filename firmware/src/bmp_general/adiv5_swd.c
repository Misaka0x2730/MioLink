...
/**
 * \brief Emit a SWD line-reset (≥50 SWCLK cycles SWDIO high) optionally followed by idle cycles.
 *
 * \param[in] idle_cycles If \c true, append four extra idle cycles after the reset (per ADIv5 §B5.3).
 */
static void swd_line_reset_sequence(const bool idle_cycles);

/**
 * \brief Walk through the JTAG→Dormant→SWD selection alert sequence to bring the DP into SWD.
 */
static void dormant_to_swd_sequence(void);

/**
 * \brief Issue the deprecated JTAG-to-SWD select sequence (16-bit pattern) and follow with line reset.
 */
...
{
    /*
     * A line reset is achieved by holding the SWDIOTMS HIGH for at least 50 SWCLKTCK cycles,
     * followed by at least two idle cycles
...
/**
 * \brief Build the 8-bit SWD packet request byte (start/APnDP/RnW/A[2:3]/parity/stop/park).
 *
 * \param[in] rnw  \c ADIV5_LOW_READ for read, \c ADIV5_LOW_WRITE for write.
 * \param[in] addr DP/AP register address; \c ADIV5_APnDP bit selects AP.
 * \return Encoded request byte ready to shift onto SWDIO.
 */
...
/**
 * \brief SWD write without ACK/parity check (TARGETSEL helper).
 *
 * \param[in] addr DP register address.
 * \param[in] data Value to write.
 * \return \c true if the response was not \c SWD_ACK_OK (i.e. an error was observed).
 */
...
/**
 * \brief SWD read without ACK/parity check.
 *
 * \param[in] addr DP register address.
 * \return Register value, or \c 0 if the ACK was not \c SWD_ACK_OK.
 */
...
/**
 * \brief Top-level SWD scan with no specific TARGETID (auto-discover).
 *
 * \return \c true if at least one DP was discovered and \c target_list is non-empty.
 */
...
/**
 * \brief Discover and initialise SWD DPs, optionally limited to a specific TARGETID.
 *
 * \param[in] targetid Target ID to bind to in multi-drop, or \c 0 to scan freely.
 * \return \c true if at least one DP was discovered and \c target_list is non-empty.
 */
bool adiv5_swd_scan_targetid(const uint32_t targetid)
{
    ...
    if (!dp_targetid) {
         ...
         *
         * It is not possible to interrogate a multi-drop Serial Wire Debug system that includes multiple devices to
         * establish which devices are connected. Because all devices are selected on coming out of a line reset, no
         * communication with a device is possible without prior selection of that target using its target ID.
         * Therefore, connection to a multi-drop Serial Wire Debug system that includes multiple devices requires
         * that either:
...
/**
 * \brief Walk all 16 SWD multi-drop instance IDs for the given TARGETID and initialise each DP found.
 *
 * \param[in,out] dp       Scratch DP to drive selection writes; freed before return.
 * \param[in]     targetid TARGETID being interrogated.
 *
 * ARM Debug Interface Architecture Specification, ADIv5.0 to ADIv5.2. ARM IHI 0031C
 *
 * §4.2.6 Limitations of multi-drop
...
void adiv5_swd_multidrop_scan(adiv5_debug_port_s *const dp, const uint32_t targetid)
{
    ...
    for (size_t instance = 0; instance < 16U; instance++) {
        /*
         * On a write to TARGETSEL immediately following a line reset sequence, the target is selected if both
...
/**
 * \brief Read a DP/AP register through SWD with posted-read RDBUFF handling.
 *
 * \param[in,out] dp   ADIv5 debug-port instance.
 * \param[in]     addr Register address; \c ADIV5_APnDP bit selects AP.
...
/**
 * \brief Clear sticky errors in \c CTRL/STAT, with optional protocol-recovery via line reset + TARGETSEL.
 *
 * \param[in,out] dp                ADIv5 debug-port instance.
 * \param[in]     protocol_recovery When \c true, perform a line-reset-based recovery sequence first.
 * \return Bits of \c CTRL/STAT.STICKY* that were cleared, or \c ADIV5_DP_CTRLSTAT_ERRMASK if recovery failed.
 */
...
/**
 * \brief Single low-level SWD transaction with WAIT/FAULT retry and exception on protocol errors.
 *
 * \param[in,out] dp    ADIv5 debug-port instance.
 * \param[in]     rnw   \c ADIV5_LOW_READ for read, \c ADIV5_LOW_WRITE for write.
 * \param[in]     addr  DP/AP register address; \c ADIV5_APnDP bit selects AP.
 * \param[in]     value Value to write (writes only).
 * \return Read response on success; \c 0 with \c dp->fault set on WAIT/FAULT/NO_RESPONSE.
 */
...
/**
 * \brief Issue an ABORT register write to the DP.
 *
 * \param[in,out] dp    ADIv5 debug-port instance.
 * \param[in]     abort \c DP_ABORT bits to assert.
 */
