 ...
 *
 * \param tms_states   Words of line data; total valid bits equals \a clock_cycles (see \c swdptap.c).
 * \param clock_cycles Total number of SWD bits to shift out.
 */
 ...
 *
 * \param request Packed ADIv5 request byte.
 * \param data    32-bit payload to write.
 ...
 *
 * \param request Packed ADIv5 request byte.
 * \param data    Receives 32-bit payload on success.
 ...
 * \param request Packed ADIv5 request byte.
 * \param data    Receives 32-bit payload on success.
 * \param parity  Receives observed parity bit as reported by the check path.
 * \return ACK/status; fails if response does not match expected parity/ACK.
 */
