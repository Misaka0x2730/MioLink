 ...
 *
 * \param pio PIO block instance.
 * \param pin Pin index (0-based) within the PIO GPIO mapping.
 */
 ...
 *
 * \param pio          PIO block instance.
 * \param sm           State machine index driving the TX FIFO.
 * \param buffer_send  Source buffer; must not be \c NULL.
 * \param data_amount  Number of 32-bit words to send; must be greater than 0 and at most \ref TAP_PIO_DMA_BUF_SIZE.
 */
 ...
 * \param pio                 PIO block instance.
 * \param sm                  State machine index.
 * \param buffer_send         Source buffer for TX; must not be \c NULL.
 * \param buffer_recv         Optional buffer for captured RX words; may be \c NULL to discard.
 * \param data_amount         Number of 32-bit words to DMA to TX; must be greater than 0 and
 *                            at most \ref TAP_PIO_DMA_BUF_SIZE.
 * \param data_amount_to_read Maximum number of RX words to store in \a buffer_recv.
 ...
 * \param pio          PIO block instance.
 * \param sm           State machine index.
 * \param buffer       Source buffer; must not be \c NULL.
 * \param data_amount  Byte count; must be greater than 0 and at most \ref TAP_PIO_DMA_BUF_SIZE.
 */
 ...
 * \param sm                  State machine index.
 * \param buffer_send         Source buffer for TX; must not be \c NULL.
 * \param buffer_recv         Optional buffer for captured RX bytes; may be \c NULL to discard.
 * \param data_amount         Bytes to DMA to TX; must be greater than 0 and at most \ref TAP_PIO_DMA_BUF_SIZE.
 * \param data_amount_to_read Maximum number of RX bytes to store in \a buffer_recv.
 ...
 *
 * Clamps \a freq to the range from \c max_interface_freq/65536 through \a max_interface_freq, computes integer/fractional
 * divider, applies it, and restarts the SM clock divider.
 *
 * \param pio                PIO block instance.
 * \param sm                 State machine index.
 * \param freq               Requested SM frequency (Hz), clamped as above.
 * \param max_interface_freq Upper frequency bound used for divider math (Hz); also caps \a freq.
