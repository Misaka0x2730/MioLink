...
{
    uint8_t *address;
} uart_dma_rx_ctrl_block_info[USB_SERIAL_UART_DMA_RX_NUMBER_OF_BUFFERS + 1] __attribute__ ((aligned (USB_SERIAL_UART_DMA_RX_NUMBER_OF_BUFFERS * sizeof(uint32_t))));
...
static void uart_rx_dma_init(const uint32_t baudrate)
{
    ...
    dma_channel_configure(uart_dma_rx_ctrl_channel,
                          &rx_ctrl_config,
                          rp_dma_get_al2_write_addr_trig(uart_dma_rx_channel),
                          uart_dma_rx_ctrl_block_info,
                          1,
                          false);
    ...
    dma_channel_configure(uart_dma_rx_channel,
                          &rx_config,
                          uart_rx_buf,
                          rp_uart_get_dr_address(current_uart),
...
static void uart_rx_dma_process_buffers(void)
{
    ...
    while (1)
    {
        ...
        const bool allow_drop_buffer = (__builtin_popcount(buffer_state) >= USB_SERIAL_UART_DMA_RX_DROP_BUFFER_THRESHOLD);
...
static bool uart_rx_dma_finish_receiving(void)
{
    ...
    while (1)
    {
        ...
        if (buffer_state & buffer_bit)
        {
            usb_serial_send_to_usb(uart_rx_buf[uart_rx_next_buffer_to_send], sizeof(uart_rx_buf[uart_rx_next_buffer_to_send]), false, true);
...
_Noreturn static void target_serial_thread(void* params)
{
    ...
    uart_rx_dma_timeout_timer = xTimerCreate("SERIAL_UART_RX",
