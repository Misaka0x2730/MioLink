...
{
	(void)(xTimer);
	xTaskNotify(usb_uart_task, USB_CDC_NOTIF_UART_RX_TIMEOUT, eSetBits);
}
...
_Noreturn static void target_serial_thread(void *params)
{
	...
	while (1) {
		if (xTaskNotifyWait(0, UINT32_MAX, &notification_value, pdMS_TO_TICKS(wait_time)) == pdPASS) {
			...
			if (notification_value & (USB_CDC_NOTIF_UART_RX_AVAILABLE | USB_CDC_NOTIF_UART_RX_FLUSH)) {
				if (notification_value & USB_CDC_NOTIF_UART_RX_AVAILABLE) {
					assert(uart_rx_use_dma == false);
					uart_rx_int_process();
				}

				if (notification_value & USB_CDC_NOTIF_UART_RX_FLUSH) {
			...
			if (notification_value & USB_CDC_NOTIF_UART_TX_COMPLETE) {
				uart_tx_dma_send();
			}

			if ((notification_value & USB_CDC_NOTIF_UART_RX_TIMEOUT) && (uart_rx_ongoing != false)) {
...
static void uart_dma_handler(void)
{
	...
	if (dma_channel_get_irq0_status(uart_dma_tx_channel)) {
		...
		xTaskNotifyFromISR(usb_uart_task, USB_CDC_NOTIF_UART_TX_COMPLETE, eSetBits, &higher_priority_task_woken);
	}
	...
	if (dma_channel_get_irq0_status(uart_dma_rx_channel)) {
		...
		xTaskNotifyFromISR(usb_uart_task, USB_CDC_NOTIF_UART_RX_FLUSH, eSetBits, &higher_priority_task_woken);
	}
