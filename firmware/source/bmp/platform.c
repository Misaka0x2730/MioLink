...
static const platform_target_pins_t miolink_target_pins = {
        .tck = MIOLINK_TARGET_TCK_PIN,
...
static const platform_led_pins_t miolink_led_pins = {
        .act = MIOLINK_LED_ACT_PIN,
        .ser = MIOLINK_LED_SER_PIN,
        .err = MIOLINK_LED_ERR_PIN
};

static const platform_vtref_info_t miolink_adc_info = {
        .enable_pin = MIOLINK_TARGET_VOLTAGE_ENABLE_PIN,
...
static const platform_vtref_info_t miolink_pico_adc_info = {
...
void make_board_ident(void)
{
    switch(platform_hwtype())
    {
        ...
        case PLATFORM_DEVICE_TYPE_MIOLINK_PICO:
            snprintf(board_ident, sizeof(board_ident), "Black Magic Probe %s%s", PLATFORM_IDENT_MIOLINK_PICO, FIRMWARE_VERSION);
...
void platform_init(void)
{
    ...
    if (p_adc_info != NULL)
    {
        ...
        if (adc_target_voltage_dma_chan == -1)
        {
            ...
            dma_channel_configure(adc_target_voltage_dma_chan,
                                  &adc_dma_config,
